...
BEGIN
  ...
  WHILE array_length(queue, 1) > 0 LOOP
    ...
    LOOP
      ...
      -- Check exclusion links for the successor (buffer-aware)
      FOR excl IN
      ...
      LOOP
        ...
        -- Compute effective end of the other task including its buffer
        ...
        IF final_s <= other_eff AND new_e >= other_start THEN
          -- Overlap detected: shift successor after the other task's effective end
      ...
      -- Add exclusion-linked tasks to the queue for re-evaluation
      FOR excl IN
        SELECT CASE WHEN task_a_id = dep.succ_id THEN task_b_id ELSE task_a_id END AS other_id
        FROM task_exclusions
        WHERE task_a_id = dep.succ_id OR task_b_id = dep.succ_id
